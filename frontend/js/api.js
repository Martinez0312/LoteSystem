// frontend/js/api.js
// ✅ Todas las variables están en window.* para ser accesibles
//    desde onclick="" en cualquier página HTML.
...
const apiFetch = async (endpoint, options = {}) => {
    ...
    try {
        ...
        if (error.name === 'SyntaxError') throw new Error('Error al procesar respuesta del servidor');
        throw error;
    }
};
...
window.API = {
    ...
    lots: {
        getAll:      (params = {}) => { const qs = new URLSearchParams(params).toString(); return apiFetch(`/lots${qs ? '?'+qs : ''}`); },
    ...
    payments: {
        ...
        downloadReceipt: (id) => {
            const token = Auth.getToken();
            fetch(`/api/payments/${id}/receipt`, { headers: { 'Authorization': `Bearer ${token}` } })
                .then(r => r.blob())
                .then(blob => {
    ...
    pqrs: {
        ...
        my:      ()            => apiFetch('/pqrs/my'),
        all:     (params = {}) => { const qs = new URLSearchParams(params).toString(); return apiFetch(`/pqrs/all${qs ? '?'+qs : ''}`); },
    ...
    initNavbar: () => {
        ...
        if (userMenuEl && user) {
            userMenuEl.innerHTML = `
                ...
                <a href="#" onclick="logout(); return false;" style="color:#dc3545;font-weight:600;">Salir</a>
            `;
        } else if (userMenuEl) {
            userMenuEl.innerHTML = `
